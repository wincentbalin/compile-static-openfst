...
ENV OPENFST_VERSION 1.6.9
ENV BAUMWELCH_VERSION 0.2.5
ENV CATEGORIAL_VERSION 1.3.3
ENV OPENGRM_NGRAM_VERSION 1.3.4
ENV THRAX_VERSION 1.2.7
...
RUN wget -q -O - http://www.openfst.org/twiki/pub/FST/FstDownload/openfst-$OPENFST_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/openfst-$OPENFST_VERSION
...
RUN wget -q -O - http://openfst.org/twiki/pub/Contrib/FstContrib/baumwelch-$BAUMWELCH_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/baumwelch-$BAUMWELCH_VERSION
...
RUN wget -q -O - http://openfst.org/twiki/pub/Contrib/FstContrib/categorial-$CATEGORIAL_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/categorial-$CATEGORIAL_VERSION
...
RUN wget -q -O - http://www.openfst.org/twiki/pub/GRM/NGramDownload/opengrm-ngram-$OPENGRM_NGRAM_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/opengrm-ngram-$OPENGRM_NGRAM_VERSION
...
RUN wget -q -O - http://invisible-mirror.net/archives/ncurses/ncurses-$NCURSES_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/ncurses-$NCURSES_VERSION
...
RUN wget -q -O - ftp://ftp.gnu.org/gnu/readline/readline-$READLINE_VERSION.tar.gz | \
    tar zxvf -
WORKDIR $C/readline-$READLINE_VERSION
